...
export default function MemberDashboardPage() {
  ...
  const testToast = () => {
    showToast('Top-left notification system active!', 'success')
  }

  if (loading) {
    return (
      <div className="container-cv py-12 text-center text-muted-foreground text-sm">
    ...
    <div className="container-cv space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 space-y-4 md:col-span-2">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className={`p-4 rounded-xl border ${profile?.role === 'user' ? 'bg-primary/5 border-primary/40' : 'bg-muted/30 border-border'}`}>
              <div className="font-bold text-foreground mb-1 flex items-center justify-between">
                ...
                {profile?.role === 'user' && <span className="text-[10px] text-primary font-bold">ACTIVE ROLE</span>}
              </div>
            ...
            <div className={`p-4 rounded-xl border ${profile?.role === 'client' ? 'bg-primary/5 border-primary/40' : 'bg-muted/30 border-border'}`}>
              <div className="font-bold text-foreground mb-1 flex items-center justify-between">
                ...
                {profile?.role === 'client' && <span className="text-[10px] text-primary font-bold">ACTIVE ROLE</span>}
              </div>
              <p className="text-muted-foreground leading-relaxed">
          ...
          {/* Test Admin Route Protection button */}
          <div className="pt-2 border-t border-border flex items-center justify-between text-xs">
            <div className="text-muted-foreground">
              Try visiting the Admin Console as a Member:
            </div>
            <Link
              href="/admin/dashboard"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-[#9e3b32]/30 bg-[#9e3b32]/10 text-[#9e3b32] font-semibold hover:bg-[#9e3b32]/20 transition-colors"
            >
